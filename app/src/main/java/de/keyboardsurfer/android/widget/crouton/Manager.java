...
final class Manager extends Handler {
		...
		if (null == croutonView.getParent()) {
			...
			if (null != crouton.getViewGroup()) {
				...
				handleTranslucentActionBar(
						(ViewGroup.MarginLayoutParams) params, activity);
				handleActionBarOverlay((ViewGroup.MarginLayoutParams) params,
						activity);
	...
	private boolean shouldAddViewWithoutPosition(ViewGroup croutonViewGroup) {
		return croutonViewGroup instanceof FrameLayout
				|| croutonViewGroup instanceof AdapterView
				|| croutonViewGroup instanceof RelativeLayout;
	}

	@TargetApi(19)
	private void handleTranslucentActionBar(
	...
	@TargetApi(11)
	private void handleActionBarOverlay(ViewGroup.MarginLayoutParams params,
			Activity activity) {
		// ActionBar overlay is only available as of Android 3.0 Honeycomb.
		if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.HONEYCOMB) {
			final boolean flags = activity.getWindow().hasFeature(
	...
	private void setActionBarMargin(ViewGroup.MarginLayoutParams params,
			Activity activity) {
		final int actionBarContainerId = Resources.getSystem().getIdentifier(
				"action_bar_container", "id", "android");
		final View actionBarContainer = activity
				.findViewById(actionBarContainerId);
